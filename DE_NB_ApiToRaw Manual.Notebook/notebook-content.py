...
guestChecks_start_date = '2024-03-21'
guestChecks_end_date = '2024-03-21'

guestChecks_off_till_start_date = '2025-01-23'
guestChecks_off_till_end_date = '2025-01-29'
guestChecks_off_till_changed_since_datetime = '2025-01-23T00:00:00Z'

menuItemDailyTotals_start_date = '2024-03-21'
menuItemDailyTotals_end_date = '2024-03-31'

kdsDetails_start_date = '2024-03-21'
kdsDetails_end_date = '2024-03-21'
...
source_entity_name = 'getPOSWasteDetails'
sink_entity_name = 'POSWasteDetails'
...
ls_date = [(start_date + timedelta(days=x)).strftime('%Y-%m-%d') 
                for x in range((end_date - start_date).days + 1)]
print_log(f"Dates to process: {ls_date}")

# udf to be applied to each location in RDD
def stage_guestChecks(locRef):
    try:
        start_time = time.time()
        print_log(f"Starting processing for location: {locRef}")    
        ...
        print_log(f"Making API request for {locRef}")
        response = requests.request("POST", url, headers=headers, data=payload)
        ...
        if response.status_code == 200:
            output_file_path = f"//lakehouse/default/Files/{sink_entity_name}/{request_date}_{locRef}_{date}_{sink_entity_name}.json"
            print_log(f"Successfully processed {locRef}")
        else:
            output_file_path = f"//lakehouse/default/Files/bad_response/{request_date}_{locRef}_{date}_{sink_entity_name}.json"
            print_log(f"Failed processing {locRef}: Status {response.status_code}", "ERROR")
        ...
        error_msg = f"Error processing {locRef}: {str(e)}"
